...
class AESpec extends FlatSpec with AE {
  ...
  // Your tests here
  "`parse2`" should "parse e3 and e4" in {
    assert(parse2("add 0 to multiply 9 by 1") == e3)
    assert(parse2("multiply 0 by add 3 to 7") == e4)
  }
  
  "`parseAdd2`" should "parse sums" in {
    assert(parseAdd2("add 1 to 1") ==
      Some((Add(Num(1), Num(1)), "")))

    assert(parseAdd2("add 1234 to 5678") ==
      Some((Add(Num(1234), Num(5678)), "")))

    assert(parseAdd2("add 1 to 1 to 1 to 1") ==
      Some((Add(Num(1), Num(1)), " to 1 to 1")))

    assert(parseAdd2("mutiply 1 by 1") == None)
    assert(parseAdd2("1 + 1") == None)
    assert(parseAdd2("add x to y") == None)
  }
  
  "`parseMul2`" should "parse products" in {
    ...
    assert(parseMul2("multiply x by y") == None)
  }
}
*/
