...
class CombinatorsSpec extends FlatSpec with Combinators {
  ...
  def testMul(parseMul: Parser[Exp]): Unit = /*???*/ {
  	val e1code = "sum of 6 and product of 6 and 6"
  	val e2code = "product of 6 and sum of 4 and 3"
  	val e3code = "product of 4 and 4 and 5"
  	val e4code = " product of 4 and 4"
  	assert(parseMul(e1code) == None)
  	assert(parseMul(e2code) == Some((e2, "")))
  	assert(parseMul(e3code) == Some( (Mul(Num(4),Num(4)), " and 5") ))
  	assert(parseMul(e4code) == None)
  }
  ...
  "`parse3`" should "parse expressions with spaces between words" in {
    ...
    assert(parse3(" sum    of     1   and 1") == Add(Num(1), Num(1)))
    assert(parse3("sum    of     1   and 10 ") == Add(Num(1), Num(10)))
    assert(parse3("sum    of     10   and 1") == Add(Num(10), Num(1)))
    
    assert(parse3(" product    of     1   and 1 ") == Mul(Num(1), Num(1)))
    assert(parse3(" product    of     1   and 1") == Mul(Num(1), Num(1)))
    assert(parse3("product    of     1   and 1 ") == Mul(Num(1), Num(1)))
    assert(parse3("product    of     1   and 1") == Mul(Num(1), Num(1)))   
	}
