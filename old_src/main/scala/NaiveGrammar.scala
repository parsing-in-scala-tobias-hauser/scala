...
object NaiveGrammar extends util.Combinators {
...
def parseGrammar(grammar: Grammar): String => Tree = input => {
  ...
  def parseRHS(ruleRHS: RuleRHS, grammar: Grammar): Parser[List[Tree]] =
    ...
    ruleRHS match {
    	...
    	case Choice(firstTry, secondTry) =>  parseRHS(firstTry, grammar) | parseRHS(secondTry, grammar)    	 		
   }
  ...
  // `parseGrammar(ae)` is not user-friendly because it produces
  ...
  //     Branch('add, List(Leaf('num, "1"), Leaf('num, "2")))
  	...
  	def deleteNode(syntaxTree: List[Tree]) : List[Tree] = syntaxTree.head match {
  		case Branch(nonterminal, children) => nonterminal match {
  			case 'exp => deleteNode(children)
  			case _ => 
  			{
  				var empty = syntaxTree.tail.isEmpty
  				empty match {
  					case true => List(Branch(nonterminal, deleteNode(children)))
  					case _ => List(Branch(nonterminal, deleteNode(children))) ::: deleteNode(syntaxTree.tail)	
  				}	
  			}
  		}
  		
  		case Leaf(terminal, string) => terminal match {
  			case 'keyword => deleteNode(syntaxTree.tail.flatMap(x => deleteNodeHelper(x)))
  			case _ => syntaxTree.flatMap(x => deleteNodeHelper(x))
  		}
  ...
  def deleteNodeHelper(x: Tree) : Option[Tree]= x match {
  	case Leaf(terminal, string) => terminal match {
  		case 'keyword => None
  		case _ => Some(x)
  	} 	
  	case Branch(nonterminal, children) => children match {
  		case List(y) => Some(y)
  	}	
  }
  ...
  def simplifyAE(syntaxTree: Tree): Tree =
   {
   		var copiedTree = deleteNode(List(syntaxTree))
   		copiedTree match {
   			case List(tree) => tree
   		}  		
   }
