...
trait Combinators extends AE {
  ...
  implicit class ParserOps[A](self: => Parser[A]) {
     ...
     *                     == choice(parser1, parser2)
     * “succeed if either the left or right operand parse successfully”
     */
    ...
    def sequence2[A, B](parseFirstPart: => Parser[A], parseSecondPart: => Parser[B]): Parser[A] =
    input => parseFirstPart(input) match {
      case Some((firstResult, afterFirstPart)) =>
        parseSecondPart(afterFirstPart) match {
          case Some((secondResult, afterSecondPart)) =>
            Some( (firstResult, afterSecondPart) )
    ...
    “succeed if the left operand is parsed successfully followed by the right, but do not include the right content in the result” */
    def <~ [B] (that: => Parser[B]): Parser[A] =
      //???
		...
		def sequence3[A, B](parseFirstPart: => Parser[A], parseSecondPart: => Parser[B]): Parser[B] =
    	input => parseFirstPart(input) match {
      	case Some((firstResult, afterFirstPart)) =>
        	parseSecondPart(afterFirstPart) match {
        	  case Some((secondResult, afterSecondPart)) =>
          	  Some( (secondResult, afterSecondPart) )
   ...
   * parse3(" sum    of     1   and 1 ") == Add(Num(1), Num(1))
   */
  def parse3(code: String): Tree = (whitespaceZero ~> exp3 <~ whitespaceZero)(code) match {
  ...
  def exp3: Parser[Exp] = add3 | mul3 | num

  def add3: Parser[Exp] =
    (/*whitespaceZero ~>*/ sum3 ~> whitespace ~> of3 ~> whitespace ~> exp3 <~ whitespace <~ and3 <~ whitespace) ~ exp3 /*<~ whitespaceZero*/ ^^ {
      case (lhs, rhs) => Add(lhs, rhs)
    }

  def mul3: Parser[Exp] =
    (whitespaceZero ~> product3 ~> whitespace ~> of3 ~> whitespace ~> exp3 <~ whitespace <~ and3 <~ whitespace) ~ exp3 <~ whitespaceZero ^^ {
      case (lhs, rhs) => Mul(lhs, rhs)
    }

  def product3: Parser[String] = parseProduct3

	def whitespace: Parser[List[String]] = oneOrMore(parseWhitespace)
	
	def whitespaceZero: Parser[List[String]] = zeroOrMore(parseWhitespace)

  def sum3: Parser[String] = parseSum3
  
  def of3: Parser[String] = parseOf3

  def and3: Parser[String] = parseAnd3
  
  
  
  // tries to match " " (whitespace)
  ...
  // tries to match "sum" to a string, and returns it
  // together with what comes after.
  def parseSum3(code: String): Option[(String, String)] = {
    val sum = "sum"
  ...
  // tries to match "product" to a string, and returns it
  // together with what comes after.
  def parseProduct3(code: String): Option[(String, String)] = {
    val product = "product"
  ...
  // tries to match "of" to a string, and returns it
  // together with what comes after.
  def parseOf3(code: String): Option[(String, String)] = {
    val of = "of"
    if (code.startsWith(of))
      Some((of, code.drop(of.length)))
    else
      None
  }
  ...
  def parseAnd3(code: String): Option[(String, String)] = {
    val and = "and"
