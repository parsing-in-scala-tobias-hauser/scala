...
trait AE {
  ...
  // 0 + (9 * 1)
  val e3 = Add(Num(0), Mul(Num(9), Num(1)))
  // assert(eval(e3) == 9)
  
  // 0 * (3 + 7)
  val e4 = Mul(Num(0), Add(Num(3), Num(7)))
  // assert(eval(e4) == 0)
  ...
  def parseMul(code: String): Option[(Mul, String)] = 
    ...
    parseProductOf(code) match {
      // code does not begin with "product of "; it's not a product.
      case None => None
            ...
            parseAnd(afterLhs) match {
              ...
              case Some((and, afterAnd)) =>
                parseExp(afterAnd) match {
                  ...
                  // succeeds to parse the entire product expression
                  case Some((rhs, rest)) =>
    ...
    parse_add(code) match {
      ...
      case Some((add, afterAdd)) =>
        parseExp2(afterAdd) match {
          ...
          case Some((lhs, afterLhs)) =>
            parse_to(afterLhs) match {
              case None => None

              // " to " matches; try to parse 2nd operand
  ...
  // tries to match " to " to a string, and returns it together
  // with what comes after.
  def parse_to(code: String): Option[(String, String)] = {
    val to = " to "
  ...
  // tries to match " by " to a string, and returns it together
  // with what comes after.
  def parse_by(code: String): Option[(String, String)] = {
    val by = " by "
  ...
  // tries to match "add " to a string, and returns it
  // together with what comes after.
  def parse_add(code: String): Option[(String, String)] = {
    val add = "add "
  ...
  // tries to match "multiply " to a string, and returns it
  // together with what comes after.
  def parse_multiply(code: String): Option[(String, String)] = {
